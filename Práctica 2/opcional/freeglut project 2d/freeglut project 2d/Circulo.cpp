...
Circulo::Circulo(const PV2D &mid, const GLdouble &radius, const GLdouble &ballRadius){
	
	const GLdouble PI = 3.14159265359;

	Circulo::_mid = mid;
	Circulo::_radius = radius;
	Circulo::_ballRadius = ballRadius;
...
bool Circulo::colision(const PV2D &p, const PV2D &v, GLdouble& tIn, PV2D& normalIn, const bool & hardMode){
	...
	GLdouble c;
	if(hardMode)
		c = cpX*cpX + cpY*cpY - pow(Circulo::_radius + Circulo::_ballRadius, 2);
	else
		c = cpX*cpX + cpY*cpY - pow(Circulo::_radius, 2);
