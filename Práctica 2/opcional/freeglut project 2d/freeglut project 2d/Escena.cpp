...
void Escena::init(const int &WIDTH, const int &HEIGHT, bool hard){
...
void Escena::createAll(const PV2D &pMid, const PV2D &move, const GLdouble &radius){
	...
	Escena::createScenery(radius);

}

void Escena::createScenery(const GLdouble &radius){
	...
	triangle = new Triangulo(pointList, radius);
	...
	Circulo *circle1 = new Circulo(PV2D(315, 125), 50, radius);
	Escena::_listaObstaculos.push_back(circle1);

	Circulo *circle2 = new Circulo(PV2D(30, 90), 40, radius);
	Escena::_listaObstaculos.push_back(circle2);

	Circulo *circle3 = new Circulo(PV2D(375, 40), 10, radius);
	Escena::_listaObstaculos.push_back(circle3);

	Circulo *circle4 = new Circulo(PV2D(550, 300), 30, radius);
	Escena::_listaObstaculos.push_back(circle4);

	Circulo *circle5 = new Circulo(PV2D(90, 350), 80, radius);
...
void Escena::step(){
	...
	list<Obstaculo*>::iterator it = Escena::_listaObstaculos.begin();
	while(it != Escena::_listaObstaculos.end()){
		it._Ptr->_Myval->colision(Escena::_ball._mid, Escena::_ball._move, tHit, nHit, Escena::_hardMode);
		if(tHit >= 0 && tMin > tHit){
			tMin = tHit;
			nMin = nHit;
		}
